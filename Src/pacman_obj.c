...
static bool pacman_movable(const Pacman* pacman, const Map* M, Directions targetDirec) {
	...
	if ((is_wall_block(M, x, y) && pacman->blockdoesntmatter != 1) || is_room_block(M, x, y))
		return false;

	return true;
}

char* get_move_path(char path[100]) {
	return path;
}
char* get_die_path(char diepath[100]) {
	return diepath;
}
...
Pacman* pacman_create() {
	...
	if (chart) {
		...
		pman->move_sprite = load_bitmap("Assets/pacman_move1.png");
		pman->die_sprite = load_bitmap("Assets/pacman_die1.png");
	}
...
void pacman_destroy(Pacman* pman) {	
	// TODO-GC-memory: free pacman resource YES
...
void pacman_draw(Pacman* pman) {
	// TODO-GC-animation: Draw Pacman and animations YES
	...
	if (!game_over) {
		...
		*/
		...
		//YES
		float time = al_get_timer_count(pman->death_anim_counter);
		game_log("%f", time);
			al_draw_scaled_bitmap(pman->die_sprite, 16* time, 0, 16, 16,
